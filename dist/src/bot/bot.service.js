...
let BotService = BotService_1 = class BotService {
        ...
        try {
            ...
            if (!config) {
                ...
                config = await this.prisma.botConfig.create({
                    data: {
                        systemPrompt: 'Eres un asistente virtual profesional y amigable para un CRM de WhatsApp. Ayuda a los usuarios de manera clara y concisa.',
                        temperature: 0.7,
                        maxTokens: 500,
                        model: 'deepseek-chat',
                        contextMessages: 5,
                        classificationCategories: ['ventas', 'soporte', 'facturacion', 'otros'],
                        orderInstructions: 'IMPORTANT: When user wants to make an order (pedido, comprar, quiero, necesito productos), you MUST follow this EXACT sequence:\n1. FIRST: Ask for products and quantities (product name and quantity) - Use prepare_order function with items\n2. SECOND: Ask for the branch name or location - Update prepare_order with branchName\n3. THIRD: When user asks for payment methods (método de pago, como pagar, forma de pago), ONLY show the available payment methods - DO NOT create the order yet\n4. FOURTH: When user CONFIRMS or SELECTS a payment method (pagaré, usaré, elegí, confirmo, listo, ok, sí, con este método, método 1, opción 1, etc.), THEN create the order in POS\n\nDO NOT create the order until the user confirms/selects a payment method. Use prepare_order to collect information step by step.\nFormat for order: List products as "product_name:quantity" (e.g., "Producto A:2, Producto B:1")',
    ...
    async updateBotConfig(data) {
        ...
        if (!config) {
            config = await this.prisma.botConfig.create({
                data: {
                    systemPrompt: data.systemPrompt || 'Eres un asistente virtual profesional y amigable para un CRM de WhatsApp. Ayuda a los usuarios de manera clara y concisa.',
                    ...
                    autoCreateOrderOnPaymentRequest: data.autoCreateOrderOnPaymentRequest ?? true,
    ...
    async generateResponse(conversationId, userMessage) {
        try {
            ...
            const functions = config.prepareOrderInsteadOfCreate ? [
                {
                    name: 'prepare_order',
                    description: 'Prepara la información de un pedido. Usa esta función paso a paso: PRIMERO cuando el usuario mencione productos (solo items), SEGUNDO cuando mencione la sucursal (agregar branchName). NO uses esta función cuando el usuario pregunte por métodos de pago - en ese caso el sistema creará el pedido automáticamente.',
                    parameters: {
                        type: 'object',
                        properties: {
                            branchName: {
